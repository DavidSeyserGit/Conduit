...
function DefaultsSettings({ settings, models, currentCodingModelId, currentJudgeModelId, updateSettings, onApply }: { settings: ReturnType<typeof useAppStore.getState>["settings"]; models: Parameters<typeof resolveQualityLanes>[0]; currentCodingModelId: string; currentJudgeModelId: string; updateSettings: (partial: Parameters<ReturnType<typeof useAppStore.getState>["updateSettings"]>[0]) => void; onApply: () => void }) {
  ...
  return <div className="space-y-5">
    <div className="space-y-3">
      <SectionTitle title="Goal preparation" description="Use a fast model to turn rough requests into structured goals and clarification questions. Reviews continue to use the selected judge model." />
      <ModelPicker label="Goal analyst" value={settings.goalAnalystModelId || currentJudgeModelId} onChange={(goalAnalystModelId) => updateSettings({ goalAnalystModelId })} isJudgePicker />
      <div className="flex items-center justify-between gap-3 text-[11px] text-gray-400">
        <span>{settings.goalAnalystModelId ? "Dedicated analyst enabled" : "Currently follows the judge model"}</span>
        {settings.goalAnalystModelId && <button type="button" onClick={() => updateSettings({ goalAnalystModelId: undefined })} className="font-medium text-gray-600 hover:text-gray-900">Use judge model</button>}
      </div>
    </div>
    <div className="space-y-5 border-t border-gray-100 pt-5">
      <SectionTitle title="Quality lane defaults" description="Each lane is a fixed worker, judge, and repair budget. Choosing it in Agent setup always applies this configuration." />
      {lanes.map((lane) => <div key={lane.id} className="rounded-xl border border-gray-200 p-3.5 space-y-3"><div className="flex items-start justify-between gap-3"><div><div className="text-sm font-semibold text-gray-800">{lane.title}</div><div className="mt-0.5 text-xs text-gray-500">{lane.description}</div></div><span className="text-[10px] font-medium text-gray-400 bg-gray-50 border border-gray-100 rounded-md px-1.5 py-0.5">{lane.iterations} loops</span></div><div className="grid grid-cols-2 gap-3"><ModelPicker label="Worker" value={lane.codingModelId} onChange={(id) => updateLane(lane.id, { codingModelId: id })} /><ModelPicker label="Judge" value={lane.judgeModelId} onChange={(id) => updateLane(lane.id, { judgeModelId: id })} isJudgePicker /></div><label className="block text-xs text-gray-700 font-medium">Repair loops<input type="number" min={1} max={10} value={lane.iterations} onChange={(event) => updateLane(lane.id, { maxIterations: Math.min(10, Math.max(1, parseInt(event.target.value) || 3)) })} className="mt-1.5 block w-20 px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-lg text-gray-900 outline-none focus:border-indigo-400" /></label></div>)}
    </div>
    <div className="pt-1 border-t border-gray-100"><div className="text-xs font-medium text-gray-700 mb-2">Custom setup fallback</div><div className="grid grid-cols-2 gap-3"><ModelPicker label="Default worker" value={defaultCoding} onChange={(id) => updateSettings({ defaultCodingModelId: id })} /><ModelPicker label="Default judge" value={defaultJudge} onChange={(id) => updateSettings({ defaultJudgeModelId: id })} isJudgePicker /></div><div className="mt-3 flex items-center justify-between gap-3"><label className="text-xs text-gray-700 font-medium">Default loops<input type="number" min={1} max={10} value={settings.defaultMaxIterations} onChange={(e) => updateSettings({ defaultMaxIterations: Math.min(10, Math.max(1, parseInt(e.target.value) || 3)) })} className="mt-1.5 block w-20 px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-lg text-gray-900 outline-none focus:border-indigo-400" /></label><button onClick={onApply} disabled={!defaultCoding || !defaultJudge} className="self-end shrink-0 px-3 py-1.5 text-xs font-medium text-white bg-gray-900 hover:bg-gray-800 disabled:bg-gray-300 rounded-lg transition-colors">Use custom now</button></div></div>
  </div>;
}
