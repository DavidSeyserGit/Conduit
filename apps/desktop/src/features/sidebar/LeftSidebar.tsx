  ...
  return (
    <aside className="w-[200px] flex flex-col bg-white border-r border-gray-100 shrink-0">
      ...
      <nav className="flex-1 px-2 py-1 space-y-0.5 overflow-y-auto">
        ...
        {runHistory.length > 0 && <>
          <div className="flex items-center justify-between px-3 pt-5 pb-1 text-[11px] font-semibold uppercase tracking-wide text-gray-400"><span>Run history</span><span>{runHistory.length}</span></div>
          {runHistory.slice(0, 8).map((entry) => {
            const resumable = entry.run.status !== "completed";
            return <div key={entry.run.id} className="group px-2 py-1">
